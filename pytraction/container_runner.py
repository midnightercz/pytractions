...
def tekton_task_name(name):
    return re.sub(r'(^|[A-Z])',
                  ...
                  if match.start() == 0
                  else '-' + match.group().lower(), name).replace('_','-')
...
def generate_task_spec(traction, docker_image, inputs_map={}, id_in_tractor=None):
    ...
    store_results_str = " ".join([f'--store-output {r["name"]}=$(workspaces.outputs.path)/{tid}::{r["name"]}' for r in results])
...
def generate_tekton_task(traction, docker_image, io_map={}, args_map={}, resource_map={}):
    ...
    result = {
        ...
        "spec": generate_task_spec(traction, docker_image, io_map=io_map, resource_map=resource_ma, args_map=args_map),
        #"results": results,
    }
    return result
...
def generate_tekton_pipeline(tractor, docker_image):
    ...
    for f, tf in tractor._fields.items():
        if f.startswith("t_"):
            ...
            for (traction, tfield), output in tractor._io_map.items():
                if f == traction:
                    if output[0] == "#":
                        ...
                        #ios_map[tfield] =  "$(workspaces.outputs.%s.results.%s)" %  (tekton_task_name(output[0]), output[1])
            ...
            task = {"name": tekton_task_name(f),
                    "taskSpec": generate_task_spec(tfo,
                                                   docker_image,
...
def generate_tekton_task_run(traction):
    ...
    for f, fv in traction._fields.items():
        if f.startswith("a_") or f.startswith("i_") or f.startswith("r_"):
            param = {"name": f,
                     "value": shift(yaml.dump(traction._fields[f].type_defaults_to_json(),
                                        explicit_start=True).rstrip())}
            params.append(param)
    result = {
        "apiVersion": "tekton.dev/v1beta1",
        "kind": "TaskRun",
