...
class Step(
    ...
    `args` - set when Step is initialized. These two variables
    ...
    `resources` - resources which are needed for step to run but are not data.
...
class NamedTractorMeta(pydantic.main.ModelMetaclass):
    def __new__(mcs, name, bases, namespace,
        ...
        for (nt_step_name, nt_step) in nt_steps:
         ...
         # not a bug, need to be se to to StepInputs to not copy step results object on setattr
    ...
    def __new__(mcs, name, bases, namespace,
        ...
        MultiDataModel = pydantic.create_model(
            ...
            **{   "__base__": StepResults,
                "multidata": (List[tractor_type.ResultsModel], [])}
        )
...
class STMD(
    ...
    def run(
        ...
        with ThreadPoolExecutor(max_workers=self.args.tractors) as executor:
            ft_results = {}
