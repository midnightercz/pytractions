...
def tractor_graph(tractor):
    ...
    for f, tf in tractor._fields.items():
        if f.startswith("t_"):
            ...
            y += max_in
            width = (max(len(getattr(tractor, f).uid), len(getattr(tractor, f).__class__.__name__))+20) * 10
            height = max(
                (len(get_traction_args(getattr(tractor, f))) + \
                len(get_traction_resources(getattr(tractor, f))))*20,
                70
            )
    ...
    for ((source, source_id), (target, target_id)) in tractor._io_map.items():
        ...
        if target != "#":
            ...
            target_t = target_id
        cells.append(make_connection(source_t, source_t + ":" + source_id, target_t, target_t + ":" + target_id))
