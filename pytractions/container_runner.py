...
export result=$(cat $(workspaces.outputs.path)/{tid}::state | python -c "import yaml; import sys; print(yaml.safe_load(sys.stdin)['data'])")
...
def generate_tekton_task_run(traction, secrets_to_volumes={}):
    ...
    if secrets_to_volumes:
        ...
        for volname, voldata in secrets_to_volumes.items():
            result['spec']['podTemplate']['volumes'].append({
                'name': volname,
                'secret': voldata
            })
...
def make_parsers(subparsers):
    ...
    p_generate_tekton_task_run.add_argument(
        "--secret-to-volume", help=f"Map secret to specific volume (required format: {json.dumps(SECRET_TO_VOLUME_SCHEMA)})", action="append", default=[]
    )
