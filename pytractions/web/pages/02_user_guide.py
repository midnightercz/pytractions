...
# 2. horizontal menu
selected2 = option_menu(None, ["List of available tractions (CLI)",
...
```
cat inputs-args-resources.yaml | python -m pytraction.runner run -m output-dir traction_module::traction_cls_name
```
...
```
If you are not sure what inputs, args and resources are needed for the traction to start you can run
```
...
helper data which tells you about data structure you need to provide for indivual input, arg or resource.

When traction is running information about it's process is in the output-dir. If you'rer running basic
traction it will have only one file named by traction uid attribute. For specialized traction like
...
""")
    ...
    st.markdown("""
If you previously ran a tractor, you can resubmit previous run from specific traction using the following command:
```
...
```
python -m pytraction.container_runner generate_tekton_task --type traction <container_image> <traction_module>:<traction_class>
```
or from container:
```
podman run <container-image> generate_tekton_task --type traction <container_image> <traction_module>:<traction_class>
```

To generate tekton pipeline for a tractor you can run following command:
```
python -m pytraction.container_runner generate_tekton_task --type tractor <container-image> <traction-module>:<traction-class>
```
or from container:
```
podman run <container-image> generate_tekton_task --type tractor <container-image> <traction-module>:<traction-class>
```
...
```
python -m pytraction.container_runner generate_tekton_task_run --type tractor|traction <traction-module>:<traction-class>
```
or from the container
```
podman run <container-image> generate_tekton_task_run --type tractor|traction <traction-module>:<traction-class>
```
