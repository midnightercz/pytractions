...
filtered_tractions, _, _, _, _, _ = catalog(tag_filter=tags_filter,
...
for d in filtered_tractions:
    for t in d['tractions']:
        tc = stylable_container(key='tractions', css_styles="""
{
border: 1px solid rgba(49, 51, 63, 0.2);
padding: calc(1em - 1px);
border-radius: 0.5rem;
background-color: #f9f9f9;
        ...
        with tc:
            ...
            tc.multiselect("Tags", key=t['name']+"-"+'tags', options=t['tags'], default=t['tags'], disabled=True)
            tc.write('Args:')
            c = args_container()
            with c:
                for targ in t['args']:
