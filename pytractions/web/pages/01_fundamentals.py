...
Input and output classes are containers for atomic data. Input attribute has to be connected only to 
output (or input) attribute of the same type. It's not possible to connect input to inner structure
defined within an output. As defined above, it's not possible to do (pseudocode):
```
T3
  input: In[str]
...
* Traction constist of 4 public type of arguments: `inputs`, `outputs`, `resources` and `args`
* Traction consists of public attribute definitions and definion of `run` method which processes inputs and produces outputs. Anything else (if needed) should be declared as private.
* Traction `run` method is supposed to process input data and produce output data. Obtaining, storing or updating data to external resources should be done using `resources`.
* All used external functionality is defined in `resources` and passed to traction when it's initialized to make testing easier.
* Traction can have zero or more inputs and zero or more outputs. It's prefered to have more outputs than
one complex output. More outputs makes traction easier to connect to other tractions.
As example, traction FoodSorter is better defined with outputs: `meat`, `vegetables`, `fruits` rather than with output `sorted_food: {meat: [], vegetables: [], fruits: []}`

* After traction process is finished, all information about it's execution are available in traction instance attributes.
* Everything publically available within a traction is json compatible [typing-system](#typing-system).

Typing system
-------------
Tractions are supposed to work with non-binary data which are consisting of json compatible
elementary types. These types are: `bool`, `string`, `int`, `float`, `list` and `dict`. Supported is also any
...
http session configuration and provide also functionality to fetch http pages. Session should be rather defined as resource which should be 
...
2. Partially initiated as a resource and final configuration should be done per traction run via traction arguments; inputs should be requested urls.
```
...
3. Partially initialized and final configuration should be passed as input model which consists of request attributes and url;
```
mhc = MyHttpClient()
...
traction run method:
    ...
    for request_data in inputs:
        mhc.fetch(request.data_url, auth=request_data.auth, retries=request_data.retries, timeout=request_data.timeout)
```

Arguments
---------
Any information which is is constant for whole traction process should be defined as argument. Those can be for
example: number of retries, timeout, threshold, number of threads etc

Resources
---------
Resources ment to provide access to external funcionality to generate, update or store data. Resource
class should be written to support elementary operations for the data. Any complex operations on the
...
Other use cases should be defined in tractions. As example of traction we can define: `RemoveInactiveClients`
...
--------------------------
Tration can be connected to each other by t2.o_input1 = t1.i_input. But to makes things reusable there's
