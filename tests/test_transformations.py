...
def test_flatten():
    t_flatten = Flatten[int](
        ...
        i_complex=In[TList[In[TList[In[int]]]]](
            data=TList[In[TList[In[int]]]](
                [
                    In[TList[In[int]]](
                        data=TList[In[int]]([In[int](data=1), In[int](data=2)])
                    ),
                    In[TList[In[int]]](
                        data=TList[In[int]]([In[int](data=3), In[int](data=4)])
                    ),
                ]
            )
        )
    ...
    assert t_flatten.o_flat == Out[TList[Out[int]]](
        data=TList[Out[int]]([Out[int](data=1), Out[int](data=2),
                              Out[int](data=3), Out[int](data=4)])
    )

def test_filter_duplicates():
    t_filter_duplicates = FilterDuplicates[int](
        uid='test-filter-duplicates',
        i_list=In[TList[In[int]]](data=TList[In[int]]([In[int](data=1), In[int](data=2), In[int](data=1)]))
    )
